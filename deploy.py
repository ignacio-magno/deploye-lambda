...
# read file ../../env.json and get variables path_credentials_mongo
def get_path_credentials_mongo():
    with open('../../env.json') as f:
        data = json.load(f)

        path_credentials = data['path_credentials_mongo']

        # if path_credentials is empty, exit 
        if path_credentials == '':
            print('path_credentials is empty')
            sys.exit()
        else:
            # if file with path path_credentials not exist, exit
            if not os.path.isfile(path_credentials):
                print('file with path path_credentials not exist')
                sys.exit() 
            else:
                # print with green color "file with path path_credentials exist"
                print('\033[92m' + 'file with path' +path_credentials + ' exist' + '\033[0m')
...
lambda_function_name = get_env_var('lambda-function-name')
role_name = lambda_function_name +"_role"
is_in_windows = sys.platform.startswith('win')
path_credentials = get_path_credentials_mongo()
...
if lambda_function_exists():
    ...
    if update == 'y':
        ...
        lambda_client.update_function_code(FunctionName=lambda_function_name, ZipFile=open('code.zip', 'rb'))
    ...
    if create == 'y':
        ...
        path_put_logs_policy = "/roles/logs.json./roles/put-logs.json"
        ...
        os.system('aws lambda create-function --function-name ' + lambda_function_name + ' --runtime go1.x --memory-size 128 --timeout 30 --role ' + role_arn + ' --handler main --zip-file fileb://main.zip')


# print thanks
print('\033[92m' + 'thanks' + '\033[0m')